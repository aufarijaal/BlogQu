<div class="bg-white dark:bg-zinc-800 shadow-sm rounded-md sm:p-6 p-3 flex flex-wrap gap-2 justify-center sm:justify-start">
    @if (!count($categories))
    ...
    @if (count($categories))
        @if ($hasPrevPage)
            <a class="bg-white w-max px-3 py-1.5 rounded-md text-sm flex gap-1 items-center shadow-sm"
                href="{{ route('search.categories', ['q' => $query, 'page' => $prevPage]) }}">
        ...
        @if ($hasNextPage)
            <a class="bg-white w-max px-3 py-1.5 rounded-md text-sm flex gap-1 items-center shadow-sm"
                href="{{ route('search.categories', ['q' => $query, 'page' => $nextPage]) }}">
                Next
