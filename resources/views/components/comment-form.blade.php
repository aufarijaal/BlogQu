<div class="p-3 bg-white dark:bg-zinc-800 shadow-sm sm:rounded-md" x-show="showComments">
    ...
    <div class="flex items-center justify-between p-3 mb-2 border rounded-md comment-reply-to border-teal-500 dark:text-white"
    ...
    <form id="form-update-comment" class="flex flex-col gap-2" action="{{ route('comments.update') }}" method="post"
        ...
        <div class="flex items-center justify-between">
            <div class="text-sm text-teal-500">Update comment</div>
            <button type="button"
                class="flex items-center justify-center w-8 h-8 transition-none rounded-full text-zinc-400 hover:bg-zinc-400 hover:text-white"
                @click="() => {
        ...
        <div class="flex flex-col gap-1">
            <textarea
                class="text-sm transition duration-150 ease-in-out border rounded-md bg-white dark:bg-zinc-800 border-zinc-300 dark:border-zinc-600 dark:text-white focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 dark:focus:ring-offset-zinc-800"
        ...
        <button class="bg-teal-500 text-white font-semibold w-[90px] h-[30px] self-end rounded-md"
    ...
    <form id="form-create-comment" class="flex flex-col gap-2" action="{{ route('comments.create') }}" method="post"
        ...
        <div class="flex flex-col gap-1">
            <textarea
                class="text-sm transition duration-150 ease-in-out border rounded-md bg-white dark:bg-zinc-800 dark:text-white border-zinc-300 dark:border-zinc-600 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 dark:focus:ring-offset-zinc-800"
                name="content" cols="30" rows="3" maxlength="200" placeholder="Write a comment..." required
                @input="inputCharCount = $el.value.length"
                @keyup.ctrl.enter="() => {
