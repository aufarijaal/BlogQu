<x-root-layout>
    ...
    <x-slot name="body">
        <div class="flex flex-col max-w-5xl min-h-screen gap-6 py-20 mx-auto">
            <div
                class="flex flex-col items-center gap-4 p-6 bg-white dark:bg-zinc-800 rounded-none shadow-sm sm:rounded-md">
                <div>
                    @if (!is_null($author->author_pp))
                        <img class="w-20 h-20 rounded-full"
                            src="{{ asset(str_contains($author->author_pp, 'http') ? $author->author_pp : '/storage/' . $author->author_pp) }}"
            ...
            <div class="flex flex-col overflow-hidden bg-white dark:bg-zinc-800 rounded-none shadow-sm sm:rounded-md"
                x-data="{ tab: 'posts' }">
                <div class="flex h-10">
                    <div :class="['flex justify-center items-center w-full font-semibold border-b cursor-pointer dark:border-zinc-600',
                        tab === 'posts' ?
                        'text-teal-500 bg-teal-50 border-teal-500 dark:bg-teal-800 dark:border-teal-700 dark:text-teal-100' :
                        'text-zinc-500'
                    ]"
