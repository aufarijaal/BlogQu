<div class="flex flex-col gap-6" x-data="{ replyTo: { name: '', content: '' }, showUpdateCommentForm: false, inputCharCount: 0 }">
    <div id="comments-section"
        class="bg-white dark:bg-zinc-800 shadow-sm sm:rounded-md flex flex-col gap-4 items-start p-6 max-h-[600px] overflow-y-auto scrollbar-thin scrollbar-track-teal-50 scrollbar-thumb-teal-500 {{ count($comments) ? 'pb-20' : '' }}"
        x-show="showComments">
        @if (count($comments))
    ...
    @else
        <div class="dark:bg-zinc-800 p-6 rounded-md flex flex-col items-center gap-4" x-show="showComments">
            ...
            <a class="block bg-teal-500 text-white rounded-md p-6 py-1 font-semibold"
