<section>
    ...
    <form method="post" action="{{ route('additional_profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-text-input id="username" name="username" type="text" class="block w-full mt-1" :value="old('username', $profile->username)" required autofocus autocomplete="username" />
        ...
        <div>
            ...
            <div class="flex gap-4">
                <label class="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300" for="gender-male">
                    <input class="accent-cyan-500" type="radio" name="gender" id="gender-male" value="M" {{ $profile->gender == 'M' ? 'checked' : '' }}>
                    Male
                </label>
                <label class="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300" for="gender-female">
                    <input class="accent-cyan-500" type="radio" name="gender" id="gender-female" value="F" {{ $profile->gender == 'F' ? 'checked' : '' }}>
                    Female
                </label>
            </div>
            <x-input-error class="mt-2" :messages="$errors->get('about')" />
